...
- Logo: `public/assets/img/fluttera-primary-logo.svg` is THE official logo (outlined mark + wordmark, currentColor, renders black). Variants: `fluttera-primary-logo-light.svg` (dark surfaces, e.g. footer), `fluttera-mark.svg` (mark only, feeds favicon/OG). Link via plain `<img>` — keep it simple. favicon/apple-touch-icon = navy tile + silver-gradient mark, generated.
